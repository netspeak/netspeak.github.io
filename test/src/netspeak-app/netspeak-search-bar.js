...
for(const phrase of collection){if(!existingElementPhraseIdsSet.has(phrase.id)){const element=this._createResultElement(phrase,collection);this._setResultElementPinned(element,phrase);this._insertResultElement(element,phrase)}}}/**
	 ...
	 * @param {HTMLElement} element
	 * @returns {HTMLElement}
	 */_addResultElementOptions(element){const phrase=this._getResultElementPhrase(element),options=(0,_util.appendNewElements)(element,"DIV.options"),examplesContainer=(0,_util.appendNewElements)(options,"DIV.examples-container");(0,_util.appendNewElements)(examplesContainer,"div.examples");const loadMoreExamplesContainer=(0,_util.appendNewElements)(examplesContainer,"div.load-more-examples"),loadingIcon=(0,_util.appendNewElements)(loadMoreExamplesContainer,"SPAN.btn-img.loading");// loading icon
...
const exampleSupplier=this._createExampleSupplier(phrase,this.examplePageSize);function loadMoreExamples(){loadingIcon.style.display=null;button.style.display="none";exampleSupplier().then(examples=>{loadingIcon.style.display="none";button.style.display=null;for(const example of examples){const p=(0,_util.appendNewElements)(examplesContainer,"DIV","P");p.innerHTML=example.snippet;(0,_util.appendNewElements)(p,"A").setAttribute("href",example.source)}}).catch(()=>{loadingIcon.style.display="none";button.style.display="none";const p=(0,_util.appendNewElements)(examplesContainer,"DIV","P");p.textContent="Failed to load examples."})}// load examples right now.
loadMoreExamples();return options}/**
	 ...
	 */_createExampleSupplier(phrase,pageSize){const pastExamples=new Set([""]),snippetsBuffer=[];/** @type {Snippet[]} */let internalPage=0,internalPageSize=2*pageSize;const snippetsApi=this.snippetsApi;/**
		 * @returns {Promise<Snippet[]>}
		 */function loadSnippets(){if(snippetsBuffer.length>=pageSize){return Promise.resolve(snippetsBuffer.splice(0,pageSize))}// load and buffer snippets
return snippetsApi.search({query:phrase.text,size:internalPageSize,from:internalPageSize*internalPage}).then(res=>{for(const _ref of res.results){const{snippet,target_uri}=_ref,text=(0,_util.textContent)(snippet).toLowerCase(),id=text.replace(/\d+/g,"0");// To be added to the buffer, the queried snippet has to both:
//  1) contain the phrase text,
//  2) not be a duplicate of a previous example.
if(text.includes(phrase.text.toLowerCase())&&!pastExamples.has(id)){pastExamples.add(id);snippetsBuffer.push({snippet:snippet+"...",source:target_uri})}}return loadSnippets()})}return loadSnippets}/**
	 * @param {HTMLElement} element
	 * @param {Phrase} phrase
