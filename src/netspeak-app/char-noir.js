define(["exports","./lru-cache.js"],function(_exports,_lruCache){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.ChatNoir=void 0;/**
 ...
 */class ChatNoir{/**
	 ...
	 */constructor(apiKey){this.apiKey=apiKey;this.baseUrl=ChatNoir.defaultBaseUrl;this.defaultIndex=ChatNoir.defaultIndexes;/** @type {LRUCache<Promise<Response>>} */this._cache=new _lruCache.LRUCache;this.caching=!0}/**
...
const req=Object.assign({},request);this._addDefaults(req);// construct URL
let url=this.baseUrl;url+="_search?q="+encodeURIComponent(req.query);url+="&apikey="+encodeURIComponent(this.apiKey);["index","from","size","explain","pretty"].forEach(p=>{if(!(p in req))return;let v=req[p];if(!0===v){url+="&"+p}else if(!1===v){// noop
}else{url+="&"+p+"="+encodeURIComponent(v+"")}});// fetch data
let fetched;if(this.caching&&this._cache.contains(url)){// console.log("cache hit");
fetched=this._cache.get(url)}else{fetched=fetch(url);if(this.caching){this._cache.add(url,fetched)}}return fetched.then(res=>res.json())}catch(error){return Promise.reject(error)}}/**
	 ...
	 */_addDefaults(request){if(!request.index)request.index=this.defaultIndex}/**
	 * The default base URL of the CharNoir API.
	 *
	 * @readonly
	 * @type {string}
	 */static get defaultBaseUrl(){return"https://webis16.medien.uni-weimar.de/chatnoir2/api/v1/"}/**
	 * The default indexes of the CharNoir API.
	 *
	 * @readonly
	 * @type {string[]}
	 */static get defaultIndexes(){return["cw09","cw12","cc1511"]}}_exports.ChatNoir=ChatNoir});