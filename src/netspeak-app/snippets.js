...
return()=>{if(noFurtherExamples){return Promise.resolve(!1)}/**@type {NetspeakSnippetsRequest} */const request={query:phrase,size:count,from:count*internalPage++};return this.search(request).then(res=>{if(0===res.results.length){noFurtherExamples=!0;return!1}/** @type {Snippet[]} */const snippets=[];res.results.forEach(({snippet,uuid,index})=>{const urls={ChatNoir:`https://www.chatnoir.eu/cache?uuid=${uuid}&index=${encodeURIComponent(index)}&plain`};snippets.push({text:(0,_util.normalizeSpaces)((0,_util.textContent)(snippet)),urls})});return snippets})}}/**
