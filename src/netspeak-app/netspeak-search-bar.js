define(["exports","meta","./netspeak-element.js","./netspeak.js","./util.js","./netspeak-navigator.js","./netspeak-example-queries.js","./snippets.js","./jsonp.js"],function(_exports,meta,_netspeakElement,_netspeak,_util,_netspeakNavigator,_netspeakExampleQueries,_snippets,_jsonp){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.PhraseFormatter=_exports.NetspeakSearchBar=void 0;meta=babelHelpers.interopRequireWildcard(meta);/**
...
message=this.localMessage("unknown-server-error",`An error occurred in Netspeak.`)}else if(details instanceof _jsonp.NetworkError){// (see jsonp.js for more info on what errors are available)
...
message=(0,_jsonp.jsonp)("https://api.github.com/orgs/github",1e4/* == 10 seconds */).then(()=>{// the request got through, so only Netspeak is offline
return this.localMessage("netspeak-unreachable-error",`The Netspeak server failed to respond. Please retry in a few minutes.`)},()=>{// the website is also offline
return this.localMessage("no-connection-error",`No response form the Netspeak server. Please make sure you have a stable internet connection.`)})}else{// everything that isn't a networking error or an error from Netspeak itself
...
message=this.localMessage("client-error",`An error occurred. Please report this to <a href="https://netspeak.org/help.html?lang=en#contact">the maintainers of Netspeak</a> should the error occur repeatedly.`)}Promise.all([message,this.localMessage("full-details","Full details")]).then(([errorMessage,fullDetails])=>{(0,_util.appendNewElements)(container,"P").innerHTML=`${errorMessage}
				<br>
				<br>
