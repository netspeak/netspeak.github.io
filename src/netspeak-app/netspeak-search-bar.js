 ...
 */class NetspeakSearchBar extends _polymerElement.PolymerElement{static get is(){return"netspeak-search-bar"}static get properties(){return{query:{type:String,value:"",observer:"_queryChanged"},corpus:{type:String,value:_netspeak.Netspeak.defaultCorpus,observer:"_corpusChanged"},initialLimit:{type:Number,value:30},readonly:{type:Boolean,value:!1},slowSearch:{type:Boolean,value:!1},initialExamplesLimit:{type:Number,value:6},historyHidden:{type:Boolean,value:!1,notify:!0,observer:"_historyHiddenChanged"}}}static get template(){return _polymerElement.html`
	 ...
	 */get pinnedPhrases(){return this._pinnedPhrases}set pinnedPhrases(value){if(Array.isArray(value))value=_netspeak.PhraseCollection.from(value);else if(!(value instanceof _netspeak.PhraseCollection))throw new TypeError("The value of pinnedPhrases has to be either an array or a PhraseCollection.");this._pinnedPhrases=value}/**
	 ...
	 */get phraseFormatter(){return this._phraseFormatter||PhraseFormatter.getDefault()}set phraseFormatter(value){this._phraseFormatter=value}/**
...
window.bar=this}/**
	 ...
	 *
	 */connectedCallback(){super.connectedCallback();this._connected=!0;/** @type {HTMLInputElement} */this._queryInputElement=this.shadowRoot.querySelector("#query-input");/** @type {HTMLButtonElement} */this._clearButton=this.shadowRoot.querySelector("#clear-button");/** @type {HTMLButtonElement} */this._historyButton=this.shadowRoot.querySelector("#history-button");this._historyHiddenChanged(this.historyHidden)}/**
	 * The method called after the element was removed from the DOM.
	 *
	 */disconnectedCallback(){super.disconnectedCallback();this._connected=!1;this._queryInputElement=this._clearButton=this._historyButton=void 0}/**
...
let newPhrases=phrases.length;this.errorMessage="";if(append){newPhrases=this._queriedPhrases.addAll(phrases)}else{this._queriedPhrases=_netspeak.PhraseCollection.from(phrases)}// show load more ?
const showLoadMore=!phrases.complete&&0<newPhrases;this.shadowRoot.querySelector("#load-more-container").style.display=showLoadMore?"block":"none";this.update(request.focusInput)}/**
	 *
...
this.shadowRoot.querySelector("#load-more-container").style.display="none";console.error(message,request);this.errorMessage=message;if(!append)this._queriedPhrases=new _netspeak.PhraseCollection;this.update(request.focusInput)}update(focusInput=!1){// declare variables
const pinnedPhrases=this.pinnedPhrases,queriedPhrases=this.queriedPhrases,pinning=!0,phrases=new _netspeak.PhraseCollection;if(pinning)phrases.addAll(pinnedPhrases);phrases.addAll(queriedPhrases);// formatter
let formatter=this.phraseFormatter;// iterate through all phrases
/** @type {HTMLElement[]} */const children=[];phrases.forEach((p,i)=>{const e=(0,_util.newElement)("DIV");e.setAttribute("text",p.text);e.setAttribute("freq",p.frequency+"");e.setAttribute("query",p.query);e.setAttribute("corpus",p.corpus);if(pinnedPhrases.includes(p))e.setAttribute("pinned","");const tr=(0,_util.appendNewElements)(e,"TABLE","TBODY","TR"),td=(0,_util.appendNewElements)(tr,"TD"),relativeFreq=Math.sqrt(p.frequency/phrases.maxFrequency);td.style.backgroundSize=100*(.618*relativeFreq)+"% 100%";(0,_util.appendNewElements)(td,"DIV","SPAN.text").innerHTML=formatter.formatText(p,phrases);const freq=(0,_util.appendNewElements)(td,"SPAN.freq");freq.innerHTML=formatter.formatFrequency(p,phrases);(0,_util.appendNewElements)(freq,"SPAN.percentage").innerHTML=formatter.formatPercentage(p,phrases);const examplesBtn=(0,_util.appendNewElements)(tr,"TD","SPAN.btn-img.examples");examplesBtn.onclick=()=>this._toggleItemOptions(e);(0,_util.appendNewElements)(examplesBtn,"SPAN.btn-img");if(pinning){const pinningBtn=(0,_util.appendNewElements)(tr,"TD","SPAN.btn-img.pinned");pinningBtn.onclick=()=>this._toggleItemPinned(e);(0,_util.appendNewElements)(pinningBtn,"SPAN.btn-img")}children.push(e)});// wrapper
/** @type {HTMLDivElement} */const wrapper=this.shadowRoot.querySelector("#result-wrapper");let showWrapper=0<children.length;// output result
/** @type {HTMLDivElement} */const resList=this.shadowRoot.querySelector("#result-list");// copy options
resList.querySelectorAll("div[text][options-visible]").forEach(e=>{const text=e.getAttribute("text"),corpus=e.getAttribute("corpus");if(!text||!corpus)return;const child=children.find(c=>c.getAttribute("text")==text&&c.getAttribute("corpus")==corpus);if(!child)return;const options=e.querySelector("div.options");if(!options)return;e.removeChild(options);child.appendChild(options);child.setAttribute("options-visible","")});// output the result
resList.innerHTML="";children.forEach(c=>resList.appendChild(c));// output errors
...
wrapper.style.display=showWrapper?"block":"none";if(focusInput&&this._queryInputElement){this._queryInputElement.focus()}}/**
	 ...
	 */_toggleItemPinned(resultListItem){const text=resultListItem.getAttribute("text");if(this.pinnedPhrases.includes(text)){this.pinnedPhrases.remove(text);resultListItem.removeAttribute("pinned")}else{const p=this.queriedPhrases.get(text);if(p){this.pinnedPhrases.add(p);resultListItem.setAttribute("pinned","")}}}/**
	 * Adds the option panel to the given result list item.
	 *
	 * @param {HTMLElement} resultListItem The item representing the phrase.
	 */_toggleItemOptions(resultListItem){if(resultListItem.hasAttribute("options-visible")){resultListItem.removeAttribute("options-visible")}else{resultListItem.setAttribute("options-visible","");if(!resultListItem.querySelector("div.options")){const e=(0,_util.appendNewElements)(resultListItem,"DIV.options"),text=resultListItem.getAttribute("text"),corpus=resultListItem.getAttribute("corpus"),wrapper=(0,_util.appendNewElements)(e,"DIV");this._loadExamples(wrapper,text,corpus)}}}/**
	 * @param {HTMLElement} e
	 * @param {string} text
	 * @param {string} corpus
	 * @param {number} [page]
...
this.snippetsApi.search({query:text,size:pageSize,from:page*pageSize}).then(res=>{this._showExamples(e,{text,corpus,page,pageSize,examples:res.results.map(r=>{const snippet=r.snippet+"...",source=r.target_uri;return{snippet:snippet,source:source}})})})}/**
	 ...
	 */_showExamples(e,examples){// remove loading
e.querySelector("SPAN.btn-img.loading").remove();// no examples found
if(0==examples.examples.length)return;// snippets that are added already
/** @type {string[]} */const existingSnippets=[];e.querySelectorAll("p.example").forEach(p=>existingSnippets.push(p.textContent||""));// add examples
let added=0;this._filterExamples(examples,existingSnippets).forEach(example=>{const p=(0,_util.appendNewElements)(e,"DIV","P.example");p.innerHTML=example.snippet;(0,_util.appendNewElements)(p,"A").setAttribute("href",example.source);added++});// load more
const text=examples.text,corpus=examples.corpus,nextPage=examples.page+1,pageSize=examples.pageSize;// load more if none were displayed
if(1>added){this._loadExamples(e,text,corpus,nextPage,pageSize);return}// add button
const btn=(0,_util.appendNewElements)(e,"BUTTON.load-more");(0,_util.appendNewElements)(btn,"SPAN.load-more-img");btn.addEventListener("click",()=>{btn.remove();this._loadExamples(e,text,corpus,nextPage,pageSize)})}/**
	 ...
	 */_filterExamples(examples,existingSnippets=[]){const res=[],text=examples.text.toLowerCase(),snippetSet=new Set(existingSnippets.map(s=>s.toLowerCase()));for(let example of examples.examples){const snippet=(0,_util.textContent)(example.snippet).toLowerCase();// does the snippet contain the text?
if(!snippet.includes(text))continue;// equal to any other snippet ?
if(snippetSet.has(snippet))continue;snippetSet.add(snippet);res.push(example)}return res}_loadMoreClick(e){/** @type {QueryPhrasesOptions} */const options={appendMode:"append",topk:this.initialLimit,searchOptions:{topkMode:"fill"}};// max frequency
...
for(let i=history.length-1;0<=i;i--){const it=history[i];if(!it||it.query==copy.query&&it.corpus==copy.corpus){history.splice(i,1)}}history.push(copy)}_historyHiddenChanged(newValue){if(!this._connected)return;this._historyButton.parentElement.style.display=newValue?"none":null}_historyButtonClick(){this._toggleHistoryDropDown()}_toggleHistoryDropDown(show=void 0){if(this.historyHidden||!this._historyButton)return;const container=this._historyButton.parentElement;if(show===void 0)show=!container.hasAttribute("history-visible");if(show){container.setAttribute("history-visible","");// current history
...
const newOpt=query=>{const opt=(0,_util.appendNewElements)(dd,"DIV.option");opt.innerHTML=query;opt.onclick=()=>{this._toggleHistoryDropDown(!1);this.query=query}};history.forEach(i=>newOpt(i.query));if(0==history.length)newOpt("");dd.focus()}else{container.removeAttribute("history-visible");container.querySelector("#drop-down").blur()}}}/** @typedef {import('./netspeak').Phrase} Phrase */ /**
 * A PhraseFormatter converts phrases into HTML source code.
 */_exports.NetspeakSearchBar=NetspeakSearchBar;class PhraseFormatter{/**
	 ...
	 */formatFrequency(phrase,collection){if(this._frequencyFormatter===void 0)this._frequencyFormatter=new Intl.NumberFormat(void 0,{style:"decimal"});const formatter=this._frequencyFormatter;let freq=phrase.frequency;// floor to 2 significant digits if the frequency has more than 3 digits
if(1e3<=freq){let log=Math.ceil(Math.log10(freq)),factor=Math.pow(10,log-2);freq=Math.floor(freq/factor)*factor}return formatter.format(freq)}/**
	 ...
	 * @param {PhraseCollection} collection The phrase collection.
	 * @returns {string} The formatted string.
	 */formatPercentage(phrase,collection){if(this._percentageFormatter===void 0)this._percentageFormatter=new Intl.NumberFormat(void 0,{style:"percent",minimumFractionDigits:1,maximumFractionDigits:1});const formatter=this._percentageFormatter;return formatter.format(phrase.frequency/collection.totalFrequency)}/**
...
classes.push((_netspeak.Word.nameOfType(w.type)+"").toLowerCase().replace(/[^a-z]+/,"-"));append("<span class=\""+classes.reduce((x,y)=>x+" "+y)+"\">"+w.text+"</span>")});return html}/**
	 ...
	 */static getDefault(){return DEFAULT_PHRASE_FORMATTER}}_exports.PhraseFormatter=PhraseFormatter;const DEFAULT_PHRASE_FORMATTER=new PhraseFormatter;window.customElements.define(NetspeakSearchBar.is,NetspeakSearchBar)});