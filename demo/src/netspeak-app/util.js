define(["exports"], function (_exports) {
  ...
  _exports.newElement = newElement;
  _exports.appendNewElements = appendNewElements;
  _exports.appendNew = appendNew;
  _exports.debounce = debounce;
  _exports.textContent = textContent;
  _exports.createNextFrameInvoker = createNextFrameInvoker;
  _exports.startScrollToUrlHash = startScrollToUrlHash;
  ...
  function newElement(selector = "SPAN") {
    // tag name
    let tagName = selector.replace(/[^\w-][\s\S]*/, "");
    if (!tagName || tagName == "*") tagName = "SPAN"; // attributes

    const attrs = [];
    const r = /\[([\w-]+)(?:=("[^"]*"|[^"][^\]]*|))?\]/g;

    for (let phrase; phrase = r.exec(selector);) attrs.push([phrase[1], phrase[2] || ""]);

    selector = selector.replace(r, ""); // id, classes
  ...
  function appendNewElements(parent, ...selectors) {
    let e = parent;

    for (let i = 0; i < selectors.length; i++) e = e.appendChild(newElement(selectors[i]));
  ...
  function appendNew(parent, children) {
  ...
  function createNextFrameInvoker(func) {
    let requested = false;
