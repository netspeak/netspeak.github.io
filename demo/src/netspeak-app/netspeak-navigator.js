define(["exports"], function (_exports) {
  ...
  _exports.NetspeakNavigator = void 0;
  ...
  class NetspeakNavigator {
    ...
    static get currentLanguage() {
      ...
      if (isValid(lang)) return lang; // lang attribute of the html tag

      lang = (document.documentElement.getAttribute("lang") || '').toLowerCase();
      if (isValid(lang)) return lang; // match against users languages

      for (let l of navigator.languages) {
        l = l.toLowerCase();
        if (isValid(l)) return l; // reduce e.g. "en-US" to "en"
    ...
    static getLanguageUrl(lang) {
      let url = location.href; // set language
  ...
  */
  ...
  _exports.NetspeakNavigator = NetspeakNavigator;
  ...
  class UrlUtil {
    ...
    static setParameter(url, name, value) {
      return UrlUtil.setParameters(url, {
        [String(name)]: value
      });
    }
    /**
