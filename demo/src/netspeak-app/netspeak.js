define(["exports", "./jsonp.js"], function (_exports, _jsonp) {
  "use strict";

  Object.defineProperty(_exports, "__esModule", {
    value: true
  });
  _exports.normalizeQuery = normalizeQuery;
  _exports.PhraseCollection = _exports.Phrase = _exports.Word = _exports.Netspeak = void 0;
  ...
  class Netspeak {
    ...
    search(request, options = {}) {
      options = Object.assign({}, options); // fill mode
      ...
      try {
        ...
        return (0, _jsonp.jsonp)(url).then(json => {
          const query = req.query;
          const corpus = req.corpus; // for information on how the JSON object is structured see www.netspeak.org
    ...
    getSearchUrl(request, baseUrl = this.baseUrl) {
      let url = baseUrl + "search?query=" + encodeURIComponent(request.query); // add the other options
    ...
    queryCorpora() {
      ...
      const result = (0, _jsonp.jsonp)(this.baseUrl + "corpus").then(json => {
    ...
    get totalFrequency() {
      if (this._total === undefined) {
        ...
        const isAdded = (words = []) => {
          ...
          if (len == 0) return false; // check for all subsets of words
  ...
  _exports.PhraseCollection = PhraseCollection;
});