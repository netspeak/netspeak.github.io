 ...
 */class Snippets{/**
	 ...
	 */getSupplier(phrase,count=6){const suppliers=this._createSuppliers(phrase,count),filter=this._createSnippetFilter(phrase);/** Whether all suppliers are empty */let empty=!1,buffer=[];/** @type {Snippet[]} */ /**
		 * @type {ResolveFn[]}
		 *
		 * @typedef {(value: Snippet[] | false) => void} ResolveFn
		 */const pendingResolves=[];/**
		 * @param {ResolveFn} resolve
		 */function resolveWhenEmpty(resolve){if(buffer.length>count){resolve(buffer.splice(0,count))}else if(buffer.length){const b=buffer;buffer=[];resolve(b)}else{resolve(!1)}}let waitingForSuppliers=!1;function callSuppliers(){if(waitingForSuppliers)return;waitingForSuppliers=!0;/** The total number of snippets returned by the suppliers this round. */let nonEmptySuppliers=0;Promise.all(suppliers.map(supplier=>{return supplier().then(snippets=>{if(snippets){// After each supplier resolves, we add all of its snippets to the buffer and check if we can
// resolve some pending resolve functions.
nonEmptySuppliers++;snippets.forEach(s=>{if(filter(s)){buffer.push(s)}});while(buffer.length>=count&&pendingResolves.length){const resolve=pendingResolves.splice(0,1)[0];resolve(buffer.splice(0,count))}}})})).then(()=>{waitingForSuppliers=!1;if(0===nonEmptySuppliers){// the suppliers are all empty
empty=!0;// resolve all pending resolve functions and clear the array
pendingResolves.forEach(resolve=>resolveWhenEmpty(resolve));pendingResolves.length=0}else{// there are still some resolve functions left, so we start again
if(pendingResolves.length)callSuppliers()}})}return()=>{return new Promise(resolve=>{if(buffer.length>=count){// there is enough in the buffer
resolve(buffer.splice(0,count))}else if(empty){resolveWhenEmpty(resolve)}else{pendingResolves.push(resolve);callSuppliers()}})}}/**
...
return()=>{if(noFurtherExamples){return Promise.resolve(!1)}/**@type {NetspeakSnippetsRequest} */const request={query:phrase,size:count,from:count*internalPage++};return this.search(request).then(res=>{if(0===res.results.length){noFurtherExamples=!0;return!1}/** @type {Snippet[]} */const snippets=[];res.results.forEach(({snippet,target_uri})=>{snippets.push({text:(0,_util.normalizeSpaces)((0,_util.textContent)(snippet)),url:target_uri})});return snippets})}}/**
