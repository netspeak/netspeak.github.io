define(["exports", "../../node_modules/@polymer/polymer/polymer-element.js", "./netspeak-navigator.js"], function (_exports, _polymerElement, _netspeakNavigator) {
  "use strict";

  Object.defineProperty(_exports, "__esModule", {
    value: true
  });
  _exports.innerHTML = innerHTML;
  _exports.loadLocalization = loadLocalization;
  _exports.highlightCode = highlightCode;
  _exports.registerElement = registerElement;
  _exports.NetspeakElement = _exports.htmlR = _exports.html = void 0;
  const html = _polymerElement.html;
  /**
  ...
  const htmlR = (strings, ...values) => {
    /** @type {any} */
    const newStrings = [...strings.raw];
    newStrings.raw = strings.raw;
    return (0, _polymerElement.html)(newStrings, ...values);
  };
  /**
  ...
  _exports.htmlR = htmlR;

  function innerHTML(htmlSource) {
    // @ts-ignore
  ...
  function loadLocalization(constructor) {
    ...
    if (!promise) {
      ...
      if (meta && meta.url && is) {
        const currentLang = _netspeakNavigator.NetspeakNavigator.currentLanguage;

        if (!noDefaultLocalization && currentLang == _netspeakNavigator.NetspeakNavigator.defaultLanguage) {
  ...
  class NetspeakElement extends _polymerElement.PolymerElement {
    constructor() {
      super(); // this is the latest we want to preload the localization
  ...
  _exports.NetspeakElement = NetspeakElement;

  function highlightCode(container) {
    if (!window.Prism || !container) return;
    const selector = ["code[class*=\"language-\"]", "pre[class*=\"language-\"] > code"].map(s => s + ":not([highlighted])").join(",");
  ...
  function registerElement(constructor) {
